...
function AdminCarousel(){
    ...
    const deleteImages = (async () => {
        ...
        await update(ref(fireDb,"Photos/HomepageCarousel"),deleteImages);

        for (var key in selectedPhotos)
        {
            const desertRef = sref(fireStorage, 'HomepageCarousel/'+selectedPhotos[key]);
            ...
            case "Upload" :
                for(let i=0;i<files.length;i++){
                    ...
                    try {
                        ...
                        await uploadBytes(sref(fireStorage,"HomepageCarousel/"+`${key}`+"/"+`${files[i].name}`),files[i]);
                        const url = await getDownloadURL(sref(fireStorage,"HomepageCarousel/"+`${key}`+"/"+`${files[i].name}`));
                        await set(ref(fireDb,"Photos/HomepageCarousel/"+key),{"Link":url});
                    } catch (error) {
    ...
    return (
        <div className="admincarousel">
            <h3>HomePage Carousel Section</h3>
            <div>
